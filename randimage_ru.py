...
def error():
    print(e);
    print(Fore.RED + 'Произошла ошибка: было введено',
        'нечисловое значение.\nПопробуйте ещё раз.');
    input();
...
while _exit:
    try:
        system('cls');
        print(Fore.YELLOW + 'Добро пожаловать в Randimage!' +
            Style.RESET_ALL);
        user_input = int(input('Введите число от 1 до 11, ' +
            'чтобы сгенерировать изображение, или 12, чтобы выйти: '));

        if user_input in [i for i in range(1, 12)]:
            system('cls');
            try:
                print('Введите путь к папке, где будет сохранён файл: ');
                p = asksaveasfilename(title='Сохранить файл',
                ...
                img.save(p);
                print(Fore.YELLOW + 'Готово! Нажмите Enter,',
